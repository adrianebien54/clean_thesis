...
The original single 1500-epoch run of each cell is pooled in as a sixth sample (see
scripts/pool_bestcombo_resolutions_6runs.py: identical config and seed, and every one of
its best-validation epochs falls inside the 800-epoch window, so the longer budget never
produced the reported checkpoint). Set INCLUDE_1500EP=0 to plot the five 800-epoch
repeats alone.

Reads $SWEEP_DIR/testset_repeats_results.json (per-rep rows written by
eval_bestcombo_repeats_testset.py), exp/combined_resolution/testset_results.json (the
1500-epoch run) and exp/combined_resolution/inference_memory.json.
...
INCLUDE_1500EP = os.environ.get("INCLUDE_1500EP", "1") != "0"
...
def build_rows() -> list[dict]:
    ...
    if INCLUDE_1500EP:
        reps = reps + json.load(ORIGINAL_JSON.open())
...
def write_table(rows: list[dict]) -> None:
    ...
    lines = [
        "# Accuracy vs peak inference memory (same-seed repeat sweep)",
        "",
        "Five 800-epoch repeats" + (" plus the original 1500-epoch run"
                                    if INCLUDE_1500EP else "") + " per cell.",
