#!/usr/bin/env python3
"""Score the 6 paper-aug selected checkpoints on the TRAIN split at 772x519.

Same protocol as scripts/eval_aug_paper_schemes_trainsplit.py, but feeds the
840 train images at 772x519 instead of the 386x260 the models were trained on —
a cross-resolution generalization check of the same checkpoints.
Predictions are unscaled with each checkpoint's training-time log_para
(25.05 CSRNet / 638.73 MobileCount); log_para cancels out of the GT count.
"""
...
DATA = ROOT / "exp/data/Tenebrio/772x519/train"
...
out = []
print(f"{'net':<12} {'arm':<9} {'epoch':>6} {'train_mae_772':>13} {'val_mae':>9} {'test_mae':>9}")
print("-" * 64)
for t in tests:
    log_para = t["log_para"]
    gt_transform = standard_transforms.Compose([own_transforms.LabelNormalize(log_para)])
    ds = Tenebrio(str(DATA), "test",
    ...
    print(f"{rec['net']:<12} {rec['arm']:<9} {rec['best_epoch']:>6} "
          f"{rec['train_mae']:>13.4f} {rec['val_mae']:>9.4f} {rec['test_mae']:>9.4f}")

    del net
    gc.collect(); torch.cuda.empty_cache()

with open(SWEEP / "trainsplit_results_772x519.json", "w") as f:
    json.dump(out, f, indent=1)
print("\nwrote", SWEEP / "trainsplit_results_772x519.json")
